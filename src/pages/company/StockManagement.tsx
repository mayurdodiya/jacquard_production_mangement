...
<div className="w-full px-4">
  <div className="grid grid-cols-[repeat(auto-fit,minmax(160px,1fr))] gap-3">
    {filteredItems.map((item) => (
      <Card
        key={item.id}
        className="w-full h-full border rounded-lg shadow-sm hover:shadow-md transition-shadow"
      >
          ...
          <div>
            ...
            <div className="space-y-1">
              <div className="flex justify-between items-start">
                ...
                <Badge className={`text-xs px-2 py-0.5 whitespace-nowrap ${getStatusColor(item.status)}`}>
                  {item.status}
                </Badge>
              </div>
            ...
            <div className="flex justify-between items-center mb-2">
              ...
              <span className="text-xs text-gray-500">
                ...
                {item.quantity <= item.minStock && (
                  <AlertTriangle className="w-3 h-3 text-yellow-600 inline ml-1" />
                )}
              </span>
            </div>
